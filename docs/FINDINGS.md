...
*Generated from snapshot `2026-08-08` by `orpulse report`. Every figure on this
page is read from `data/marts/`; nothing is typed by hand.*

**Scope of this capture:** 532 model-variants, 258.71 T
tokens and 16.43 B requests over the trailing 30 days.
...
|---|---|---|---|---|---|---|
| **agentic** | 60 | 187.15 T | 72.3% | 51.6 | 47,228 | large contexts, terse output, very large interactions |
| **conversational** | 263 | 68.31 T | 26.4% | 10.5 | 3,940 | moderate context per output token, human-sized interactions |
| **unclassified** | 105 | 2.32 T | 0.9% | — | 29 | insufficient data to classify |
| **extractive** | 26 | 805.88 B | 0.3% | 41.0 | 8,584 | context-heavy but small interactions: classification, extraction, routing |
| **output_heavy** | 78 | 132.54 B | 0.1% | 0.6 | 4,411 | emits one token per two consumed; in practice almost entirely image-output models |

Models classified as *agentic* account for **72.3% of all tokens**
while being 60 of 532 model-variants. Conversational
...
|---|---|---|---|---|
| `meta-llama/llama-guard-4-12b` | 243.6 | 899 | 17.60 B | extractive |
| `nvidia/nemotron-3-ultra-550b-a55b-20260604` | 161.0 | 104,741 | 11.28 T | agentic |
| `poolside/laguna-s-2.1-20260720` | 160.5 | 95,007 | 2.54 T | agentic |
| `poolside/laguna-m.1-20260312` | 136.6 | 68,991 | 1.16 T | agentic |
| `xiaomi/mimo-v2.5-20260422` | 116.5 | 67,822 | 34.18 T | agentic |
| `poolside/laguna-xs-2.1-20260625` | 97.2 | 55,892 | 596.34 B | agentic |
| `anthropic/claude-opus-5-fast-20260723` | 95.7 | 84,074 | 84.15 B | agentic |
| `stepfun/step-3.7-flash-20260528` | 85.6 | 67,903 | 5.89 T | agentic |
| `anthropic/claude-4.8-opus-fast-20260528` | 85.0 | 58,124 | 82.14 B | agentic |
| `xiaomi/mimo-v2.5-pro-20260422` | 80.7 | 56,672 | 2.52 T | agentic |

**Why both axes.** The top of this ranking is `meta-llama/llama-guard-4-12b` at
243.6 tokens of context per token written, but its interactions
average only 899 tokens. A high P:C ratio alone
cannot tell a coding agent from a safety classifier; both read far more than they
write. Reading a lot per call and reading a lot per token produced are different
properties, and only their conjunction identifies agentic use.


Contrast `nvidia/nemotron-3-ultra-550b-a55b-20260604`: 161.0 tokens of context per token
written, in interactions averaging 104,741 tokens, which
is 116× larger.
...
median of **0.91** with a p25–p75 range of 0.65–1.12.
...
|---|---|---|---|---|
| `openai/gpt-5.6-luna-20260709` | 3.91× | 5.91 T | 30 | agentic |
| `openai/gpt-5.6-terra-pro-20260709` | 3.55× | 122.29 B | 30 | agentic |
| `openai/gpt-5.6-luna-pro-20260709` | 3.35× | 792.52 B | 30 | agentic |
| `z-ai/glm-5v-turbo-20260401` | 3.02× | 37.12 B | 129 | agentic |
| `tencent/hy3-20260706` | 2.76× | 13.86 T | 33 | agentic |
| `google/gemini-3.6-flash-20260721` | 2.65× | 2.86 T | 18 | agentic |
| `openai/gpt-5.6-terra-20260709` | 2.55× | 1.48 T | 30 | agentic |
| `qwen/qwen3-235b-a22b-thinking-2507` | 2.29× | 18.62 B | 379 | conversational |

**Fading** — lowest momentum among ratable models:

| Model | Momentum | Tokens (30d) | Age (days) | Archetype |
|---|---|---|---|---|
| `ibm-granite/granite-4.0-h-micro` | 0.02× | 36.99 B | 292 | agentic |
| `perceptron/perceptron-mk1-20260512` | 0.04× | 10.40 B | 88 | extractive |
| `amazon/nova-2-lite-v1` | 0.09× | 16.56 B | 249 | conversational |
| `mistralai/mistral-medium-3` | 0.16× | 4.28 B | 458 | conversational |
| `openai/gpt-4o-2024-05-13` | 0.16× | 3.75 B | 817 | extractive |
| `google/gemma-2-27b-it` | 0.17× | 660.23 M | 756 | conversational |
| `openai/o4-mini-2025-04-16` | 0.18× | 48.70 B | 479 | conversational |
| `deepseek/deepseek-v3.1-terminus` | 0.22× | 173.14 B | 320 | conversational |

## 4. Attention and money are different markets

Multiplying each model's tokens by its list price gives the gross value its
traffic represents: **$172.1 M per month** across
376 priced model-variants.
...
|---|---|---|---|
| `anthropic` | 9.6% | 44.4% | 4.63x |
| `openai` | 8.5% | 15.0% | 1.78x |
| `moonshotai` | 2.5% | 8.4% | 3.32x |
| `z-ai` | 6.3% | 7.0% | 1.10x |
| `deepseek` | 19.1% | 6.3% | 0.33x |
| `google` | 8.4% | 5.5% | 0.66x |
| `xiaomi` | 14.4% | 3.5% | 0.24x |
| `tencent` | 12.3% | 2.5% | 0.21x |

Concentration makes the same point without naming a winner. Measured by tokens,
the labs sit at an HHI of **1,061**. Measured by money they sit at
**2,887**, past the 2,500 mark competition authorities treat as
...
Of 651 endpoints serving multi-provider models,
**419 are dominated** (64.4%).

| Model | Dominated endpoint | $/M out | p50 throughput | Beaten by | # better |
|---|---|---|---|---|---|
| `deepseek/deepseek-v4-flash-20260731` | Mancer 2 | $0.50 | 15 tok/s | DeepInfra | 19 |
| `~deepseek/deepseek-v4-flash-latest` | Mancer 2 | $0.50 | 15 tok/s | DeepInfra | 19 |
| `deepseek/deepseek-v4-flash-20260731` | SiliconFlow | $0.28 | 7 tok/s | DeepInfra | 18 |
| `~deepseek/deepseek-v4-flash-latest` | SiliconFlow | $0.28 | 7 tok/s | DeepInfra | 18 |
| `deepseek/deepseek-v4-flash-20260731` | Venice | $0.35 | 18 tok/s | DeepInfra | 16 |
| `~deepseek/deepseek-v4-flash-latest` | Venice | $0.35 | 18 tok/s | DeepInfra | 16 |
| `deepseek/deepseek-v4-flash-20260731` | AkashML | $0.28 | 17 tok/s | DeepInfra | 15 |
| `~deepseek/deepseek-v4-flash-latest` | AkashML | $0.28 | 17 tok/s | DeepInfra | 15 |
...
|---|---|---|---|---|---|---|
| **agentic** | request weighted | -0.43 | -0.66 to -0.20 | 0.332 | 52 | yes |
| **all** | request weighted | -0.13 | -0.50 to +0.25 | 0.007 | 351 | no |
| **conversational** | request weighted | -0.26 | -0.73 to +0.21 | 0.024 | 238 | no |
| **extractive** | request weighted | -0.08 | -1.11 to +0.95 | 0.003 | 20 | no |
| **output_heavy** | request weighted | +0.06 | -0.19 to +0.32 | 0.008 | 40 | no |
| **agentic** | unweighted | -0.20 | -0.70 to +0.29 | 0.011 | 52 | no |
| **all** | unweighted | -0.53 | -0.81 to -0.25 | 0.037 | 351 | yes |
| **conversational** | unweighted | -0.67 | -1.00 to -0.35 | 0.066 | 238 | yes |
| **extractive** | unweighted | -0.00 | -0.37 to +0.37 | 0.000 | 20 | no |
| **output_heavy** | unweighted | -0.81 | -1.41 to -0.21 | 0.075 | 40 | yes |

**The reversal in agentic traffic is the result worth the space.** Counting
models equally, price explains nothing (-0.20, interval
-0.70 to +0.29, straddling zero). Weighting by requests, the
elasticity is **-0.43** (-0.66 to -0.20) and
...
|---|---|---|---|---|
| ≥2 days silent | 21 | 357 | 97.7% | 93.4% |
| ≥3 days silent | 18 | 360 | 98.2% | 93.9% |
| ≥7 days silent | 18 | 360 | 98.2% | 93.9% |
| ≥14 days silent | 12 | 366 | 98.9% | 96.2% |
